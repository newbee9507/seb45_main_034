...
public class RatingController {
    ...
    public void saveRating(@RequestBody RatingDTO ratingDTO,
        ...
        setMovieAverageRating(ratingDTO.getMovieId());
    }

    private void setMovieAverageRating(Long movieId) {

        Movie movie = movieService.findMovie(movieId);

        List<Rating> ratings = ratingService.sendRating(movieId);
        DoubleStream tmp = ratings.stream().mapToDouble(Rating::getRating);
        movie.setAverageRating(tmp.average().orElse(0));
    }
}
